...
void Instruction_parsing_char(char *server_char_addr, struct Instruction *parsing, char segmentation){
  ...
  do {
    ...
    if(3 == AT_Number_instructions)
      data_addr = Instruction_parsing_Split((int)(p), (int)&(parsing->buf), 0);
    else
      data_addr = Instruction_parsing_Split((int)(p), (int)&(parsing->buf), segmentation);	// 解包
