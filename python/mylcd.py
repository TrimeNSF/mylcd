...
class Menu:
    ...
    def get_option(self):
        if self.name == lang["Play Speed"]:
            ...
            with open('/home/pi/data/speed.data') as fd:
            ...
            with open('/home/pi/data/bright.data') as fd:
            ...
            with open('/home/pi/data/MACAddr.data') as fd:
                MAC = list(fd.readline())
            MAC.insert(4,'-')
            MAC.insert(9,'-')
            return ''.join(MAC)
        elif self.name == lang["MAC(BCD)"]:
            with open('/home/pi/data/MACAddrBCD.data') as fd:
                MAC = list(fd.readline())
            MAC.insert(4,'-')
            MAC.insert(9,'-')
            return ''.join(MAC)
            ...
            with open('/home/pi/data/version.data') as fd:
                line = fd.readline().split('\n')[0]           #!! will remove \n
            SysVer = line.split(' ')[0]
...
def switch_refresh_rate(options, current_option):
    print('current option is : %s' %(options[current_option]))
    os.system('echo %d > /home/pi/data/refresh.data' % current_option)
...
def load_info():
    ...
    while 1:
        time.sleep(0.09)
        '''
        if mtime_1 != os.path.getmtime("/home/pi/data/1.data"):
            mtime_1 = os.path.getmtime("/home/pi/data/1.data")
            fd = open('/home/pi/data/1.data')
            Sum = int(fd(readline().split('\n')[0])
            fd.close()
        '''
        if mtime_1 != os.path.getmtime("/home/pi/data/filelist1.txt"):
            print("file list change...")
            file_options.clear()
            mtime_1 = os.path.getmtime("/home/pi/data/filelist1.txt")
            with open('/home/pi/data/filelist1.txt') as fd:
                lines = fd.readlines()
            for line in lines:
        ...
        if mtime_video != os.path.getmtime("/home/pi/data/video.data"):
            print("video change...")
            mtime_video = os.path.getmtime("/home/pi/data/video.data")
            with open('/home/pi/data/video.data') as fd:
                 video = int(fd.readline().split('\n')[0])
            dict_menu_options['File'].current_option = video

        if mtime_speed != os.path.getmtime("/home/pi/data/speed.data"):
            print("speed change...")
            mtime_speed = os.path.getmtime("/home/pi/data/speed.data")
            with open('/home/pi/data/speed.data') as fd:
                 speed = int(fd.readline().split('\n')[0])
            dict_menu_options['Play Speed'].current_option = speed - 1

        if mtime_bright != os.path.getmtime("/home/pi/data/bright.data"):
            print("brightness change...")
            mtime_bright = os.path.getmtime("/home/pi/data/bright.data")
            with open('/home/pi/data/bright.data') as fd:
                 bright = int(fd.readline().split('\n')[0])
            dict_menu_options['Brightness'].current_option = bright

        if mtime_refresh != os.path.getmtime("/home/pi/data/refresh.data"):
            print("refresh rate change...")
            mtime_refresh = os.path.getmtime("/home/pi/data/refresh.data")
            with open('/home/pi/data/refresh.data') as fd:
